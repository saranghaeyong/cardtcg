...
import { saveCustomCard } from '../utils/storage';
        ...
        <form onSubmit={handleSubmit} className="flex-1 w-full flex flex-col gap-4 text-xs font-mono">
          <div>
            ...
            <p className="text-slate-400 font-sans text-xs mt-1">
              Mint custom collectible cards with your own real people photos and stats.
            </p>
          </div>
          ...
          <div>
            <label className="block text-slate-300 font-bold mb-1.5 flex items-center gap-1">
              <Upload className="w-3.5 h-3.5 text-sky-400" /> Photo (Upload or Preset)
            </label>
          ...
          <button
            type="submit"
            className="mt-2 w-full py-3 rounded-xl bg-gradient-to-r from-sky-400 via-indigo-500 to-purple-500 hover:from-sky-300 hover:to-purple-400 text-slate-950 font-black tracking-widest text-xs uppercase flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-indigo-500/30 transition-all"
          >
            <Plus className="w-4 h-4" />
            <span>MINT CARD & ADD TO PACK POOL</span>
          </button>
        </form>
      </div>
    </div>
